...
def plot_teardown_adoption(conn, out_dir, show):
    ...
    fig.suptitle("Fixture Cleanup/Teardown Adoption (Resource Management Discipline)", 
    ...
    bars1 = ax.bar(x, teardown_counts[:, 0], width, label="No Teardown",
                   color="#e74c3c", alpha=0.8)
    bars2 = ax.bar(x, teardown_counts[:, 1], width, bottom=teardown_counts[:, 0],
                   label="With Teardown", color="#2ecc71", alpha=0.8)
    
    ax.set_ylabel("Number of Fixtures")
    ax.set_title("8f: Teardown Adoption by Language (Stacked)", fontsize=11, fontweight='bold')
    ax.set_xticks(x)
    ...
    colors = ['#2ecc71' if p >= 50 else '#f39c12' if p >= 25 else '#e74c3c' for p in adoption_pct]
    ...
    ax.set_title("8f: Teardown Adoption Rate by Language", fontsize=11, fontweight='bold')
    ax.set_xlim(0, 105)
    ...
    stat_text += f"With Teardown: {total_with:,} ({100*total_with/len(fixtures):.1f}%)\n"
