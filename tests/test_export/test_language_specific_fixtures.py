...
def test_export_language_specific_fixtures(temp_db):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            df_python.iloc[0]["github_url"] == expected_url
        ), f"GitHub URL mismatch: got {df_python.iloc[0]['github_url']}"
        ...
        assert (
            df_java.iloc[0]["github_url"] == expected_java_url
...
def test_fixture_csv_has_expected_columns():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                df.iloc[0]["github_url"] == expected_url
            ), f"GitHub URL mismatch: {df.iloc[0]['github_url']}"
